...
class FilterGene:
    ...
    def __init__(self, junction_table, output_path) -> None:
        ...
        assert all([x in self.df.columns for x in ['junc_id', 'hgnc']]),\
    ...
    def run(self):
        ...
        with open(os.path.join(self.output_path, "gene_exclusion_intention.tsv"), "w") as file_handle:
            intention_df = self.df[['junc_id', 'hgnc'] + list(self.filter_regex.keys())]
            intention_df.to_csv(file_handle, index=False, sep="\t")
        
        with open(os.path.join(self.output_path, "sj_problematic_gene.tsv"), "w") as fail_handle, \
                open(os.path.join(self.output_path, "sj_pass_gene.tsv"), "w") as pass_handle:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Filter SJ for problematic gene loci",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog=epilog)
    parser.add_argument('-i', '--input', dest='input', help='Specify input splice junction table', required=True)
    parser.add_argument('-o', '--output', dest='output', help='Specify output file', required=True)
    args = parser.parse_args()

    filter = FilterGene(args.input, args.output)
    filter.run()
