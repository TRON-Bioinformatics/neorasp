...
class CpmNormalization:
    ...
    def get_seq_depth(self, mapped=False) -> int:
        ...
        with open(log_file, "r") as file_handle:
            for line in file_handle:
                if line.split("|")[0].strip() == "Number of input reads":
                ...
                elif line.split("|")[0].strip() == "Number of reads mapped to multiple loci":
                    ...
                    continue
        if mapped:
            return number_of_uniquely_mapped_reads + number_of_multimapping_reads + number_of_chimeric_reads
        else:
    ...
    def _read_sj_out_tab(self) -> pd.DataFrame:
        ...
        junctions = pd.read_csv(self.sj_out,
                          sep='\t',
                          names=self.columns_sj_out,
                          dtype=self.sj_out_dtypes)
        return junctions
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate splice junction Count Per Million (CPM) value",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        epilog=epilog)
    parser.add_argument('-i', '--input', dest='input', help='Specify input SJ.out.tab', required=True)
