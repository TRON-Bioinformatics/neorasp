...
def get_final_output():
    ...
    for sample in samples.itertuples():
        ...
        final_files.extend(
            collect(
                "results/{sample}/fetchdata/sj_final.tsv", sample=sample.sample_name
            )
        )
        final_files.extend(
            collect(
        ...
        final_files.extend(
            collect(
                "results/{sample}/star/{sample}_Aligned.sortedByCoord.out.cram",
                sample=sample.sample_name,
            )
        )
...
def get_fq(wildcards):
    ...
    """
    fq = samples.query(
        "sample_name == @wildcards.sample & replicate == @wildcards.replicate"
    )
    fq1 = fq.get("fq1").item()
    fq2 = fq.get("fq2").item()
...
def get_replicates(wildcards):
    ...
    return (
        samples.query("sample_name == @wildcards.sample").get("replicate", None).values
    )
...
def get_star_input(wildcards):
    ...
    return {
        "fq1": [
            f"results/{wildcards.sample}/fastp/{rep}/{wildcards.sample}_R1.fastq.gz"
            for rep in replicates
        ],
        "fq2": [
            f"results/{wildcards.sample}/fastp/{rep}/{wildcards.sample}_R2.fastq.gz"
            for rep in replicates
        ],
    }
...
def get_rg_star(wildcards):
    ...
    return " , ".join(
        [
            f"ID:{wildcards.sample}_{rep}\tSM:{wildcards.sample}\tPL:ILLUMINA"
            for rep in replicates
        ]
    )
...
def get_multiqc_input(wildcards) -> list:
    ...
    for sample in samples.itertuples():
        ...
        final_files.extend(
            collect(
                "results/{sample}/metrics/{sample}.inner_distance_freq.txt",
                sample=sample.sample_name,
            )
        )
        final_files.extend(
            collect(
                "results/{sample}/metrics/{sample}.junctionSaturation_plot.r",
                sample=sample.sample_name,
            )
        )
        final_files.extend(
            collect(
                "results/{sample}/metrics/{sample}.read_distribution.txt",
                sample=sample.sample_name,
            )
        )
        final_files.extend(
            collect(
                "results/{sample}/metrics/{sample}.featureCounts.txt.summary",
                sample=sample.sample_name,
            )
        )
        final_files.extend(
            collect(
                "results/{sample}/star/{sample}_Log.final.out",
                sample=sample.sample_name,
            )
        )
    return final_files
...
def aggregate_splice2neo_output(wildcards) -> dict:
    ...
    splice2neo_final_files["peptide_junc"] = expand(
        ...
        chunkID=glob_wildcards(
            os.path.join(checkpoint_output, "splice2neo_input_{chunkID}")
        ).chunkID,
    )
