...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Add wild-type hits of novel junction sequences.\n",
    )

    parser.add_argument(
        "--input_splice", required=True, help="splice2neo annotation table"
    )
    parser.add_argument(
        "--output",
        required=True,
    ...
    parser.add_argument(
        "--transcriptome",
        required=True,
        help="Reference transcriptome to blast against.",
    )
    parser.add_argument("--temp_dir", default="/tmp", help="tmp directory")
    parser.add_argument(
        "--mismatch", default=0, help="Number of mismatches allowed in blast alignment."
    )
    parser.add_argument(
        "--threads",
        default=1,
    ...
    input_df = pd.read_csv(input_splice, sep="\t", low_memory=False, comment="#")
    ...
    temp["stop"] = temp.apply(
        lambda x: min(x.cts_junc_pos + 20, len(x.cts_seq)), axis=1
    )
    temp["query_sequence"] = temp.apply(lambda x: x.cts_seq[x.start : x.stop], axis=1)
    temp["query_cts_id"] = temp.apply(
        lambda x: xxhash.xxh64(x.query_sequence).hexdigest(), axis=1
    )
    # position - alter_start + neuer
    temp["junc_in_query"] = temp.apply(
        lambda x: x.cts_junc_pos - (x.start + 1) + 1, axis=1
    )
    ...
    temp = temp.loc[
        (temp.junc_in_query > temp.qstart) & (temp.junc_in_query < temp.qend)
    ]
    temp.to_csv(f"{out_file}.tmp", sep="\t", index=False)
    if not len(temp.index) == 0:
        temp["wt_match"] = temp.apply(
            lambda x: f"{x.wt_transcript}:{int(x.tstart)}:{int(x.tend)}:{int(x.length)}/{len(x.query_sequence)}",
            axis=1,
        )
        temp = (
            temp.loc[:, ["cts_id", "wt_match"]]
            .drop_duplicates()
            .groupby("cts_id")["wt_match"]
            .apply(";".join)
            .reset_index()
        )
